...
{
    ChildMap(YAML::Emitter& emitter, rttr::string_view key)
        : YAMLBase(emitter)
    {
...
void loadSimpleProperty(YAML::Node& node, const rttr::type& propertyType, rttr::property& property, rttr::instance& instance)
{
    bool success = false;
...
void loadProperty(YAML::Node& node, rttr::property property, rttr::instance instance)
{
    ...
    else if (propertyType.is_arithmetic() || 
    ...
    else if (propertyType.is_associative_container())
    {
        if (node.IsSequence())
        {
            ...
            const rttr::type valueType = associativeView.is_key_only_type() ? associativeView.get_key_type() : associativeView.get_value_type();
        ...
        if (node.IsSequence())
        {
            ...
            for (YAML::Node seq : node)
            {
                if (seq.IsScalar())
                {
                    ...
                    {
                        sequentialView.insert(sequentialView.begin() + static_cast<int>(sequentialView.get_size()), loadedVariant);
                    }
                }
                ...
                else if (seq.IsMap())
                {
                    ...
                    rttr::variant variant = needToCreate ? valueType.create() : sequentialView.get_value(currentIndex);

                    assert(variant.is_valid());

                    for (rttr::property childProperty : valueType.get_properties())
                    {
                    ...
                    if (needToCreate)
                    {
                        rttr::variant_sequential_view::const_iterator iterator = sequentialView.insert(sequentialView.begin() + currentIndex, variant);

                        assert(iterator != sequentialView.end());
                    }
                    else
                    {
                        bool set = sequentialView.set_value(currentIndex, variant);
                        assert(set);
                    }
                }
            }
    ...
    else if (propertyType.is_class())
    {
        if (node.IsMap())
        {
            ...
            for (rttr::property classProperty : classVariant.get_type().get_properties())
            {
    ...
    {
        assert(false);  // ignore the assert, just want to see what we haven't accounted for
        loadSimpleProperty(node, propertyType, property, instance);
    }
}
...
rttr::instance SB::Core::Serialization::Serializer::createAndLoadObject(YAML::Node& node, std::optional<rttr::method> onLoadedMethod)
{
    rttr::type type = rttr::type::get_by_name(node[s_ObjectTypeKey].as<std::string>());
...
bool SB::Core::Serialization::Serializer::loadProperties(YAML::Node& node, rttr::instance instance, std::optional<rttr::method> onLoadedMethod)
{
    bool result = false;
...
bool Serializer::saveVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant& variant)
{
    ...
    if (variant && emitter.good())
    {
        ...
        if (type.is_arithmetic() || type.is_wrapper() || type == rttr::type::get<std::string>() || type == rttr::type::get<std::string_view>())
        {
...
bool SB::Core::Serialization::Serializer::saveStringVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant variant)
{
    bool result = false;
...
bool SB::Core::Serialization::Serializer::saveEnumVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant variant)
{
    bool result = false;
...
bool SB::Core::Serialization::Serializer::saveAssociateContainerVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant variant)
{
    bool result = false;
    ...
    if (emitter.good() && name.size() && variant.is_valid())
    {
        ...
        if (!view.is_empty() && valueType.is_valid())
        {
            ...
            {
                const std::pair<rttr::variant, rttr::variant> valuePair = item.convert<std::pair<rttr::variant, rttr::variant>>();

                rttr::variant key = valuePair.first.extract_wrapped_value();
                rttr::variant value = view.is_key_only_type() ? valuePair.first.extract_wrapped_value() : valuePair.second.extract_wrapped_value();
...
bool SB::Core::Serialization::Serializer::saveSequentialContainerVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant variant)
{
    bool result = false;
...
bool SB::Core::Serialization::Serializer::saveClassVariant(YAML::Emitter& emitter, rttr::string_view name, rttr::variant variant)
{
    bool result = false;
