...
VoiceTracker* VoiceTracker::get()
{
	return s_voiceTracker;
}
...
unsigned int VoiceTracker::getPlayingCountOfObject(SB_ID id) const
{
	...
	if (std::unordered_map<SB_ID, unsigned int>::const_iterator find = m_nodePlayingCount.find(id); find != m_nodePlayingCount.cend())
	{
		result = find->second;
	}
