...
void SB::Engine::NodeInstance::setSoundInstance(SoundContainer* soundContainer, Sound* sound) noexcept
{
	...
	ma_result attach = ma_node_attach_output_bus(sound->getSound(), 0, m_nodeGroup.get()->m_tail->m_state->m_userData, 0);
...
{
	if (SB::Engine::SoundContainer* const soundContainer = m_referencingNode->tryConvertObject<SB::Engine::SoundContainer>())
	{
