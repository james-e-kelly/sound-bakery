...
void SB::Engine::Voice::playContainer(Container* container) noexcept
{
	...
	for (Container* const containerToPlay : containersToPlay)
	{
		...
		if (SoundContainer* const soundContainer = containerToPlay->tryConvertObject<SoundContainer>())
		{
			if (Sound* const sound = soundContainer->getSound())
			{
				std::unique_ptr<NodeInstance>& voiceInstance = m_voiceInstances.emplace_back(std::make_unique<NodeInstance>());
...
bool SB::Engine::Voice::isPlaying() const
{
	return m_voiceInstances.size();
}
