...
void SB::Engine::GameObject::postEvent(Event* event)
{
	if (event)
	{
		for (const Action& action : event->m_actions)
		{
			...
			if (const SB::Core::DatabasePtr<SB::Core::DatabaseObject>& destination = action.m_destination; destination.lookup())
			{
...
bool SB::Engine::GameObject::isPlaying() const noexcept
{
	return voiceCount();
}
