...
{ 
	...
	assert(constructor.is_valid() && "Object in Sound Bakery must be constructable. Define this in the Reflection file");

	rttr::variant variant = constructor.invoke();
	assert(variant.is_valid());
	assert(variant.get_type().is_pointer() && "Database Objects must be constructed as raw ptrs. Raw objects or shared ptrs is disallowed");
