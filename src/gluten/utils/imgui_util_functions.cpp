#include "imgui_util_functions.h"
...
ImRect gluten::imgui::rect_offset(const ImRect& rect, float x, float y)
{
    ImRect result = rect;
    result.Min.x += x;
    result.Min.y += y;
...
bool gluten::imgui::update_window_manual_resize(ImGuiWindow* window, ImVec2& newSize, ImVec2& newPosition)
{
    ...
    auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow* window, const ImVec2& size_contents)
    {
        ...
        else
        {
            ...
            if (is_popup ||
                is_menu)  // Popups and menus bypass style.WindowMinSize by default, but we give then a non-zero minimum
...
bool gluten::imgui::begin_menubar(const ImRect& barRectangle)
{
    ...
    ImRect bar_rect = gluten::imgui::rect_offset(barRectangle, 0.0f, padding.y);  // window->MenuBarRect();
...
bool gluten::imgui::button_centered(const char* label, const ImVec2& size)
{
