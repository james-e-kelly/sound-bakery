...
namespace gluten::imgui
{
    struct scoped_style
	{
		...
		template<typename T>
		scoped_style(ImGuiStyleVar styleVar, T value) { ImGui::PushStyleVar(styleVar, value); }
	...
	struct scoped_color
	{
		...
		template<typename T>
		scoped_color(ImGuiCol ColorId, T Color) { ImGui::PushStyleColor(ColorId, Color); }
	...
	struct scoped_id
	{
		...
		template<typename T>
		scoped_id(T id) { ImGui::PushID(id); }
