...
typedef struct SC_DSP_STATE
{
    ...
    void* m_userData;       //< points to uer created object, likely some type of ma_node
...
struct SC_DSP
{
    ...
    SC_DSP_VTABLE*  m_vtable;   // holds the functions for interacting with the underlying node type. Must be not null

    SC_DSP_TYPE     m_type;

    SC_DSP*         m_next;     // when in a node group, the parent/next dsp. Can be null if the head node
    SC_DSP*         m_prev;     // when in a node group, the child/previous dsp. Can be null if the tail node
};
