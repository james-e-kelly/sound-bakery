...
static SC_RESULT SC_DSP_FADER_CREATE(SC_DSP_STATE* state)
{
	...
	if (state->m_userData == NULL) return MA_OUT_OF_MEMORY;
...
static SC_RESULT SC_DSP_LOWPASS_CREATE(SC_DSP_STATE* state)
{
	...
	ma_lpf_node_config config = ma_lpf_node_config_init(ma_engine_get_channels((ma_engine*)state->m_system), ma_engine_get_sample_rate((ma_engine*)state->m_system), SC_DSP_CUTOFF_MAX, SC_DSP_DEFAULT_FILTER_ORDER);
...
static SC_RESULT SC_DSP_LOWPASS_GET_PARAM_FLOAT(SC_DSP_STATE* state, int index, float *value)
{
	(void)state;
	(void)value;
...
static SC_RESULT SC_DSP_HIGHPASS_CREATE(SC_DSP_STATE* state)
{
	...
	ma_hpf_node_config config = ma_hpf_node_config_init(ma_engine_get_channels((ma_engine*)state->m_system), ma_engine_get_sample_rate((ma_engine*)state->m_system), SC_DSP_CUTOFF_MIN, SC_DSP_DEFAULT_FILTER_ORDER);
...
static SC_RESULT SC_DSP_HIGHPASS_GET_PARAM_FLOAT(SC_DSP_STATE* state, int index, float* value)
{
	(void)state;
	(void)value;
