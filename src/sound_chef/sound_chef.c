    ...
    ma_node* node, const float** framesIn, ma_uint32* const frameCountIn, float** framesOut, ma_uint32* frameCountOut)
{
    ...
    if (clapPlugin->start_processing(clapPlugin))
    {
        ...
        {
            deinterleavedInputFrames[channel] =
                ma_malloc(ma_get_bytes_per_sample(ma_format_f32) * *frameCountIn,
                          &((ma_engine*)clapNode->baseNode.pNodeGraph)->allocationCallbacks);
            deinterleavedOutputFrames[channel] =
                ma_malloc(ma_get_bytes_per_sample(ma_format_f32) * *frameCountOut,
                          &((ma_engine*)clapNode->baseNode.pNodeGraph)->allocationCallbacks);
        }
        ...
        for (int channel = 0; channel < inputChannels; ++channel)
        {
            ma_free(deinterleavedInputFrames[channel],
                    &((ma_engine*)clapNode->baseNode.pNodeGraph)->allocationCallbacks);
            ma_free(deinterleavedOutputFrames[channel],
                    &((ma_engine*)clapNode->baseNode.pNodeGraph)->allocationCallbacks);
        }
    }
}
