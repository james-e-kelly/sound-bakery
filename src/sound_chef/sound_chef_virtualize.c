...
static float sc_voice_calculate_audibility(const sc_voice* voice)
{
    // Placeholder — extend with 3D falloff × listener distance × group gain
    // × obstruction × occlusion once those layers exist. For now, gain is a
    // reasonable stand-in so the machinery is exercisable end-to-end.
...
sbk_status sc_system_calculate_virtual_voices(sc_system* system)
{
    ...
    // Pass 1 — gather. vol0 candidates auto-virtualise inline; everyone else
    // is stashed and histogrammed by priority.
    for (sc_uint32 i = 0; i < maxVoices; ++i)
    {
        ...
        if (desired != sc_voice_desired_playing || current == sc_voice_state_stopped || current == sc_voice_state_stopping)
        {
            continue;
        }
    ...
    // Pass 2 — walk histogram top-down to find the priority cutoff and how
    // many candidates at exactly `cutoff` still fit.
