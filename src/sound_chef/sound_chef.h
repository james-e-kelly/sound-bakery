/**
 * @file
 * @brief A wrapper library for miniaudio that adds types like NodeGroup to act similarly to an FMOD ChannelGroup.
 */
...
typedef enum SC_DSP_TYPE
{
    ...
    SC_DSP_TYPE_FADER,      // ma_engine_node. Calling it a fader to match its use in busses
    SC_DSP_TYPE_LOWPASS,
 ...
 * 
 * Nodes in the group go from left to right, tail to head. Imagine a snake and everything is moving towards the endpoint/device.
 * Any input to the group goes to the tail and all outputs leave from the head.
 * 
 * Nodes can be inserted in any position. The specified index becomes the index for the inserted node.
 * Index 0 is the tail.
*/
struct SC_NODE_GROUP
{
    SC_DSP* m_tail;             // left/bottom most node in the group. sounds and child groups connect to this. will equal m_fader upon startup
    SC_DSP* m_fader;            // the ma_sound_group object which sits in the middle of the group. effects are added either side of it. handles volume, pitching etc       
    SC_DSP* m_head;             // right/top most node in the group. every node in the group routes to this before going to a parent group or endpoint
};
 ...
 * This means that SC_SYSTEM has a node graph, resource manager, can output to the user's audio device and everything expected from miniaudio's high-level API.
 * 
 ...
 * 
 * Initializes the ma_engine and therefore the ma_device. This makes the system connect to the user's audio device and
 * is then ready for playing sounds.
*/
 ...
 * 
 * Creates a new Node Group with a SC_DSP_TYPE_FADER by default (for handling volume, pitch etc.).
 ...
 * Using SC_DSP_Config_Init will ensure the vtable is correct for built in types.
 * However, users must fill it themselves for custom DSP types.
 * 
 * Example:
 * @code
 * SC_DSP* dsp = NULL;
 ...
 * 
 * @warning Will not return a valid config if the type == SC_DSP_TYPE_UNKOWN.
*/
...
/**
 * @name DSP
 * DSP is an abstraction over specific effects and the user doesn't need to know the structure of the effect, only its type and any parameters they want to change.
 * This simplifies the design and API for higher level uses like UI and serialization as only become interested in its type and parameter indexes.
 * @{
*/
...
/**
 * @brief Attaches this group to a parent where this group outputs its audio to the parent.
 * 
