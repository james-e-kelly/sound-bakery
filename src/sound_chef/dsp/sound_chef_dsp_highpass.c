...
static sbk_status sc_dsp_highpass_create(sc_system* system, sc_dsp* dsp, void* userData)
{
    ...
    return SBK_FROM_MA(ma_hpf_node_init((ma_node_graph*)system, &config, &system->engine.allocationCallbacks, (ma_hpf_node*)dsp->node));
}
...
static sbk_status sc_dsp_highpass_set_param_float(sc_dsp* dsp, sc_uint32 index, float value)
{
    ...
    switch (index)
    {
        ...
        case SC_DSP_HIGHPASS_PARAM_CUTOFF:
        {
            ...
            result = SBK_FROM_MA(ma_hpf_node_reinit(&hpfConfig, dsp->node));
            break;
        }
    }
