...
auto review_database::create_review_version(database_id reviewId, const new_transit_review_data newReviewVersion, std::string userToken) const -> database_result<review_data>
{
    ...
    if (!contextFilesMap.empty())
    {
        ...
        std::transform(contextFilesMap.begin(), contextFilesMap.end(), result.m_relativeContextFiles.begin(), [](const auto& pair)
                       { return pair.second; });
    }
    ...
    if (!reviewFilesMap.empty())
    {
        ...
        std::transform(reviewFilesMap.begin(), reviewFilesMap.end(), result.m_reviewAssets.begin(), [](const auto& pair)
                       { return pair.second; });
    }
    ...
    auto create_files = [&databasePath, &thisReviewPath](const std::vector<review_file_data>& reviewFiles, std::vector<versionable_review_asset>& result)
    {
        ...
        for (const auto& rawContextFile : reviewFiles)
        {
            if (!uniqueFileNames.contains(rawContextFile.m_fileName))
            {
                ...
                const auto existingFileIter = std::find_if(result.begin(), result.end(), [fileName = rawContextFile.m_fileName](const versionable_review_asset& asset)
...
auto review_database::get_all_reviews(database_id projectId, std::string userToken) const -> database_result<std::vector<review_data>>
{
    ...
    while (query.executeStep())
    {
        ...
        if (!contextFilesMap.empty())
        {
            ...
            std::transform(contextFilesMap.begin(), contextFilesMap.end(), reviewData.m_relativeContextFiles.begin(), [](const auto& pair)
                           { return pair.second; });
        }
        ...
        if (!reviewFilesMap.empty())
        {
            ...
            std::transform(reviewFilesMap.begin(), reviewFilesMap.end(), reviewData.m_reviewAssets.begin(), [](const auto& pair)
                           { return pair.second; });
        }
...
auto review_database::get_all_review_activity(database_id reviewId, std::string userToken) const -> database_result<std::vector<activity_data>>
{
    ...
    std::sort(result.begin(), result.end(), [](const auto& lhs, const auto& rhs)
              { return lhs.m_activityId > rhs.m_activityId; });
...
auto review_database::get_all_comments(database_id reviewId, database_id commentId, std::string userToken) const -> database_result<std::vector<comment_data>>
{
    ...
    std::sort(result.begin(), result.end(), [](const auto& lhs, const auto& rhs)
              { return lhs.m_commentId > rhs.m_commentId; });
