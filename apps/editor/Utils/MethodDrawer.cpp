...
{
	if (!instance.get_type().is_derived_from(rttr::type::get<SB::Core::Object>()))
	{
		assert(false);
		return;
	}

	SB::Core::DatabaseObject* object = instance.try_convert<SB::Core::DatabaseObject>();
	...
	for (const rttr::method& method : type.get_methods())
	{
		...
		if (ImGui::Button(method.get_name().data()))
		{
			...
			case 5:
				...
				break;
			case 6:
				method.invoke(instance, arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
				break;
			}
		}
