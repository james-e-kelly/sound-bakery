...
ButtonState FileBrowserWidget::DrawWideButton(bool selected,
...
void FileBrowserWidget::ShowItemContextMenu(const std::filesystem::path& path) const noexcept
{
	...
	if (ImGui::BeginTable("Dir Browser Table", 1, ImGuiTableFlags_Resizable | ImGuiTableFlags_RowBg))
	{
		...
		if (m_currentDirectory.is_absolute())
		{
			for (auto& p : std::filesystem::directory_iterator(m_currentDirectory))
			{
				...
				const auto btn_state = DrawWideButton(selected, btn_hovered_color, btn_active_color);
				...
				if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID))
				{
					...
					ImGui::SetDragDropPayload(SB::Editor::PayloadSound.c_str(), m_draggingFile.c_str(), (m_draggingFile.size() + 1) * sizeof(char), ImGuiCond_Once);
