...
void ProjectManager::Init(const ProjectConfiguration&& projectConfig)
{
    ...
    // Widgets
    GetApp()->GetSubsystemByClass<WidgetSubsystem>()->AddWidgetClassToRoot<ProjectExplorerWidget>();
    GetApp()->GetSubsystemByClass<WidgetSubsystem>()->AddWidgetClassToRoot<PlayerWidget>();
    GetApp()->GetSubsystemByClass<WidgetSubsystem>()->AddWidgetClassToRoot<DetailsWidget>();
    ...
    m_previewSoundContainer = rttr::rttr_cast<SB::Engine::SoundContainer*, SB::Core::Object*>(SB::Engine::Factory::createObjectFromType(rttr::type::get<SB::Engine::SoundContainer>()));
    m_previewSoundContainer->setDatabaseName("Preview Node");
    SB::Core::ObjectTracker::get()->untrackObject(m_previewSoundContainer);
    m_previewSoundContainerResource = SB::Core::Database::get()->removeUnsafe(m_previewSoundContainer);
...
void ProjectManager::SaveYAMLToFile(YAML::Emitter& yaml, std::ofstream& stream, std::filesystem::path file) const
{
...
void ProjectManager::SaveObjectToFile(YAML::Emitter& yaml, std::ofstream& stream, SB_ID objectID, std::filesystem::path file, std::string_view fileExtension) const
{
    if (yaml.good() && yaml.size() > 0)
    {
        std::filesystem::path nodeFile =  file / (std::to_string(objectID) + fileExtension.data());
...
void ProjectManager::SaveProject() const
{
    ...
    SaveYAMLToFile(systemYaml, outputStream, m_projectConfiguration.m_projectFolder / "System.yaml");

    for (const std::weak_ptr<SB::Core::DatabaseObject>& object : SB::Core::Database::get()->getAll())
    {
        if (std::shared_ptr<SB::Core::DatabaseObject> sharedObject = object.lock())
        {
            ...
            SaveObjectToFile(yaml, outputStream, sharedObject->getDatabaseID(), m_projectConfiguration.m_objectsFolder / SB::Util::TypeHelper::getFolderNameForObjectType(sharedObject->getType()), ".yaml");
        }
    }
}
...
void ProjectManager::LoadProject()
{
    ...
    std::vector<std::filesystem::path> loadPaths { m_projectConfiguration.m_objectsFolder };
    ...
    for (const std::filesystem::directory_entry& p : std::filesystem::directory_iterator(m_projectConfiguration.m_projectFolder))
    {
    ...
    for (const std::filesystem::directory_entry& p : std::filesystem::recursive_directory_iterator(m_projectConfiguration.m_sourceFolder))
    {
        if (p.is_regular_file())
        {
            const std::filesystem::path filename = p.path().filename();

            if (SB::Core::Database::get()->tryFind(filename.string().c_str()) == nullptr)
            {
                if (SB::Core::DatabaseObject* const createdSound = newDatabaseObject<SB::Engine::Sound>())
                {
                    ...
                    if (SB::Engine::Sound* const castedSound = rttr::rttr_cast<SB::Engine::Sound*, SB::Core::DatabaseObject*>(createdSound))
                    {
